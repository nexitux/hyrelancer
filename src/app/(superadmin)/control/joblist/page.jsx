...
const JobListingPage = () => {
    ...
    const handleRestore = async (jobId) => {
        ...
        try {
            ...
            setJobs(prev => prev.map(j => j.cuj_id === jobId ? { ...j, cuj_is_active: 1, cuj_is_rejected: 0 } : j));
        } catch (err) {
    ...
    const getStatusBadge = (job) => {
        ...
        switch (status) {
            ...
            default:
                return <span className="px-2 py-1 text-xs rounded-full bg-yellow-100 text-yellow-700">Unknown</span>;
        }
    };
    ...
    const filteredJobs = jobs.filter(job => {
        const matchesSearch =
            ...
            job.job_category?.sc_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            job.job_service?.se_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="border-b bg-slate-50 border-slate-200">
                            <tr>
                                <th className="px-6 py-3 text-sm font-semibold text-left text-gray-700">Job</th>
                                <th className="px-6 py-3 text-sm font-semibold text-left text-gray-700">Category</th>
                                <th className="px-6 py-3 text-sm font-semibold text-left text-gray-700">Customer</th>
                            ...
                            {filteredJobs.length > 0 ? (
                                filteredJobs.map((job) => (
                                    <tr key={job.cuj_id} className="transition-colors hover:bg-slate-50">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex gap-3 items-center">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm text-slate-500">ID: {job.cuj_id}</p>
                                        ...
                                        <td className="px-6 py-4 text-right whitespace-nowrap">
                                            ...
                                            {(() => {
                                                const isSoftDeleted = job.cuj_is_rejected === 0 && job.cuj_is_active === 0; // both 0 => show Restore and disable others
                                                const activeButtonEnabled = job.cuj_is_rejected === 0 && job.cuj_is_active === 1; // enable Active only for this condition
                                                ...
                                                return (
                                                    <div className="flex gap-2 justify-end items-center">
                                                        ...
                                                        <button
                                                            className={`p-2 rounded-lg transition-colors ${activeButtonEnabled ? 'text-green-600 hover:bg-green-50' : 'text-slate-300 cursor-not-allowed'}`}
                                                            title="Active"
                                                            onClick={() => activeButtonEnabled && handleToggleStatus(job.cuj_id, job.cuj_is_active)}
                                                        ...
                                                        {/* Freelancers list */}
                                                        <Link
                                                            href={`/control/jobs/freelancers/${encodeId(job.cuj_id)}`}
                                                            className={`p-2 rounded-lg transition-colors ${isSoftDeleted ? 'text-slate-300 pointer-events-none' : 'text-purple-600 hover:bg-purple-50'}`}
                                ...
                                <tr>
                                    <td colSpan="6" className="px-6 py-12 text-center">
                                        ...
                                        <p className="text-slate-600">Try adjusting your search criteria or add a new job.</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>

...
export default function Page({ params }) {
  ...
  useEffect(() => {
    ...
    const fetchProfileData = async () => {
      ...
      try {
        ...
        // Check if profile is not found or not verified
        if (data.message && data.message === "Please choose a valid user" && data.ustatus === "0") {
          setError("No profile found or profile not verified");
          setProfileData(null);
          return;
        }
  ...
  if (error) {
    // Special handling for profile not found or not verified
    if (error === "No profile found or profile not verified") {
      return (
        <div className="min-h-screen bg-gray-50 py-8 px-4 flex items-center justify-center">
          <div className="text-center max-w-md mx-auto">
            <div className="bg-gray-100 border border-gray-300 rounded-lg p-8">
              ...
              <h2 className="text-xl font-semibold text-gray-800 mb-2">{capitalizeWords("Profile Not Found")}</h2>
              <p className="text-gray-600 mb-6">{capitalizeFirst("This profile does not exist or has not been verified yet.")}</p>
            </div>
          </div>
        </div>
      );
